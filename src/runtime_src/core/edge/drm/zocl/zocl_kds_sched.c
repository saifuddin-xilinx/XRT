...
void zocl_kds_sched_init(struct zocl_drm_dev *zdev, void *sched_handlr)
{
	/* FIXME */
}

void zocl_kds_sched_release(struct zocl_drm_dev *zdev, void *sched_handlr)
{
	/* FIXME */
}
...
{
	struct zocl_kds_sched_manager	*zsched_manager = NULL;

	zsched_manager = vzalloc(sizeof(struct zocl_kds_sched_manager));
	if (!zsched_manager)
		return -ENOMEM;

	zsched_manager->sched_ops = &zocl_kds_sched_ops;
