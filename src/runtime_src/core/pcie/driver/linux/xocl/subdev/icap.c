...
struct icap {
	...
	icap_slot_info		*icap_slot[ICAP_MAX_SLOT];
	void			*rp_bit;
	size_t			rp_bit_len;
	void			*rp_fdt;
	size_t			rp_fdt_len;
...
{
	BUG_ON(icap->icap_xclbin[slot_id]->icap_bitstream_ref < 0);
	return icap->icap_xclbin[slot_id]->icap_bitstream_ref != 0;
}
